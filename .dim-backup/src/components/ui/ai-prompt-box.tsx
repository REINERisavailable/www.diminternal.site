...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant = "default", size = "default", ...props }, ref) => {
        ...
        const sizeClasses = {
            ...
            sm: "h-8 px-3 text-sm",
            lg: "h-12 px-6",
            icon: "h-8 w-8 rounded-full aspect-[1/1] flex-shrink-0",
        };
        return (
            <button
                className={cn(
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
    const { onSend = () => { }, isLoading = false, placeholder = "Type your prompt or upload an image...", className } = props;
    ...
    const handleSubmit = () => {
        if (input.trim() || files.length > 0) {
            onSend(input, files.length > 0 ? files[0] : null);
        }
    };
